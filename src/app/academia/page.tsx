...
export default function AcademiaLoginPage() {
  ...
  useEffect(() => {
    // Extrair slug da URL: /academia/power-fit/ → power-fit
    const path = window.location.pathname;
    const parts = path.split('/').filter(Boolean);
    const academiaIndex = parts.indexOf('academia');
    if (academiaIndex >= 0 && parts[academiaIndex + 1]) {
      const s = parts[academiaIndex + 1];
      setSlug(s);
      loadAcademia(s);
    } else {
      setNotFound(true);
  ...
  const loadAcademia = async (s: string) => {
